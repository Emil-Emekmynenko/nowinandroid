...
class MainScreenTestHm15 : TestCase(
    Kaspresso.Builder.withComposeSupport(),
) {
    @get: Rule
    val composeTestRule = createAndroidComposeRule<MainActivity>()
    val mainScreenHm15 = MainScreenHm15(composeTestRule)
    val searchScreenHm15 = SearchScreenHm15(composeTestRule)

    @Test
    fun checkButtonTest() {
        run {
            step("Click button Allow") {
                device.uiDevice.findObject(
                    UiSelector().text("Allow"),
                ).click()
            }
            mainScreenHm15 {
                step("Check hasText main title") {
                    titleText.assertIsDisplayed()
                    titleText.assertTextEquals("Now in Android")
                }
                step("Check button search") {
            ...
            searchScreenHm15{
                ...
                step("Check text input search"){
                        searchText.assertIsDisplayed()
                        searchText.performTextInput("Lebron")
                    }
                }
            }
        }
